...
async function resetMongo() {
  ...
  // Track how many playlists each song appears in
  const playlistCountMap = new Map();
    ...
    if (playlist.songs && Array.isArray(playlist.songs)) {
      playlist.songs.forEach((song) => {
        ...
        if (
          ...
          // Increment playlist count for this song
          playlistCountMap.set(key, (playlistCountMap.get(key) || 0) + 1);
  ...
  // Now set the correct inPlaylists count for each song
  const uniqueSongs = Array.from(songMap.values()).map((song) => ({
    ...song,
    inPlaylists: playlistCountMap.get(song.youTubeId) || 0,
  }));

  console.log(`Extracted ${uniqueSongs.length} unique songs from playlists`);
